...
import AttributeAutocompleteView from './ui/attributeautocompleteview.js';
import DomWrapperView from './ui/domwrapperview';
import AttributeAutocompleteListItemView from './ui/attributeautocompletelistitemview.js';
 ...
 */
export default class AttributeAutocompleteUI extends Plugin {
	/**
	 * @inheritDoc
	 */
	static get pluginName() {
		return 'AttributeAutocompleteUI';
	}
	...
	constructor( editor ) {
		 ...
		 * @private
		 */
		this._attributeAutocompleteView = this._createAttributeAutocompleteView();
		 ...
		 * @private
		 */
		this._attributeAutocompleteConfigurations = new Map();
		...
		editor.config.define( 'attributeautocomplete', { feeds: [] } );
	}
	...
	init() {
		...
		editor.editing.view.document.on( 'keydown', ( evt, data ) => {
			if ( isHandledKey( data.keyCode ) && this._isUIVisible ) {
				...
				if ( data.keyCode == keyCodes.arrowdown ) {
					this._attributeAutocompleteView.selectNext();
				}

				if ( data.keyCode == keyCodes.arrowup ) {
					this._attributeAutocompleteView.selectPrevious();
				}

				if ( data.keyCode == keyCodes.enter || data.keyCode == keyCodes.tab || data.keyCode == keyCodes.space ) {
					this._attributeAutocompleteView.executeSelected();
				}
		...
		clickOutsideHandler( {
			emitter: this._attributeAutocompleteView,
			activator: () => this._isUIVisible,
			contextElements: [ this._balloon.view.element ],
			callback: () => this._hideUIAndRemoveMarker()
		} );

		const feeds = editor.config.get( 'attributeautocomplete.feeds' );

		for ( const attributeAutocompleteDescription of feeds ) {
			const feed = attributeAutocompleteDescription.feed;

			const marker = attributeAutocompleteDescription.marker;

			if ( !isValidAttributeAutocompleteMarker( marker ) ) {
				/**
				...
				throw new CKEditorError(
					'attributeautocompleteconfig-incorrect-marker: The marker must be provided and it must be a single character.',
					null
				);
			}

			const minimumCharacters = attributeAutocompleteDescription.minimumCharacters || 0;
			const feedCallback = typeof feed == 'function' ? feed.bind( this.editor ) : createFeedCallback( feed );
			const watcher = this._setupTextWatcherForFeed( marker, minimumCharacters );
			const itemRenderer = attributeAutocompleteDescription.itemRenderer;

			const definition = { watcher, marker, feedCallback, itemRenderer };

			this._attributeAutocompleteConfigurations.set( marker, definition );
		}
	...
	destroy() {
		...
		this._attributeAutocompleteView.destroy();
	}
	 ...
	 */
	get _isUIVisible() {
		return this._balloon.visibleView === this._attributeAutocompleteView;
	}
	 ...
	 */
	_createAttributeAutocompleteView() {
		const locale = this.editor.locale;

		const attributeAutocompleteView = new AttributeAutocompleteView( locale );

		this._items = new Collection();

		attributeAutocompleteView.items.bindTo( this._items ).using( data => {
			const { item, marker } = data;

			const listItemView = new AttributeAutocompleteListItemView( locale );
			...
			listItemView.on( 'execute', () => {
				attributeAutocompleteView.fire( 'execute', {
					item,
					marker
				} );
			} );
		...
		attributeAutocompleteView.on( 'execute', ( evt, data ) => {
			const editor = this.editor;
			...
			const attributeAutocompleteMarker = editor.model.markers.get( 'attributeautocomplete' );

			// Create a range on matched text.
			const end = model.createPositionAt( model.document.selection.focus );
			const start = model.createPositionAt( attributeAutocompleteMarker.getStart() );
		...
		return attributeAutocompleteView;
	}
	...
	_getItemRenderer( marker ) {
		const { itemRenderer } = this._attributeAutocompleteConfigurations.get( marker );
	...
	_requestFeed( marker, feedText ) {
		...
		const { feedCallback } = this._attributeAutocompleteConfigurations.get( marker );
	...
	_setupTextWatcherForFeed( marker, minimumCharacters ) {
		...
		watcher.on( 'matched', ( evt, data ) => {
			...
			if ( checkIfStillInCompletionMode( editor ) ) {
				const mentionMarker = editor.model.markers.get( 'attributeautocomplete' );
				...
				editor.model.change( writer => {
					writer.addMarker( 'attributeautocomplete', { range: markerRange, usingOperation: false, affectsData: false } );
				} );
			}
	...
	_handleFeedResponse( data ) {
		...
		const mentionMarker = this.editor.model.markers.get( 'attributeautocomplete' );
	...
	_showOrUpdateUI( markerMarker ) {
		if ( this._isUIVisible ) {
			...
			this._balloon.updatePosition( this._getBalloonPanelPositionData( markerMarker, this._attributeAutocompleteView.position ) );
		} else {
			this._balloon.add( {
				view: this._attributeAutocompleteView,
				position: this._getBalloonPanelPositionData( markerMarker, this._attributeAutocompleteView.position ),
				withArrow: false,
				singleViewMode: true
			} );
		}

		this._attributeAutocompleteView.position = this._balloon.view.position;
		this._attributeAutocompleteView.selectFirst();
	}
	...
	_hideUIAndRemoveMarker() {
		...
		if ( this._balloon.hasView( this._attributeAutocompleteView ) ) {
			this._balloon.remove( this._attributeAutocompleteView );
		}

		if ( checkIfStillInCompletionMode( this.editor ) ) {
			this.editor.model.change( writer => writer.removeMarker( 'attributeautocomplete' ) );
		}

		// Make the last matched position on panel view undefined so the #_getBalloonPanelPositionData() method will return all positions
		// on the next call.
		this._attributeAutocompleteView.position = undefined;
	}
...
function hasExistingMention( position ) {
	...
	const hasMention = position.textNode && position.textNode.hasAttribute( 'attributeautocomplete' );

	const nodeBefore = position.nodeBefore;

	return hasMention || nodeBefore && nodeBefore.is( 'text' ) && nodeBefore.hasAttribute( 'attributeautocomplete' );
}
...
// @param {String} marker
// @returns {Boolean}
function isValidAttributeAutocompleteMarker( marker ) {
...
function checkIfStillInCompletionMode( editor ) {
	return editor.model.markers.has( 'attributeautocomplete' );
}
